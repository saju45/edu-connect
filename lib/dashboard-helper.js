...
import { getCourseDeatails, getCourseDetailsByInstructor } from "@/queries/courses";
    ...
    const populatedEnrollments = await Promise.all(
      enrollments.map(async (enrollment) => {
        ...
        if (report) {
          ...
          // Calculate progress
          const course = await getCourseDeatails(enrollment?.course?._id);
...
export async function getInstructorDashboardData(dataType) {
    try {
        ...
        console.log("getInstructorDashboardData : ",data);
